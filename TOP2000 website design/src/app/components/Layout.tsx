...
export function Layout() {
  ...
  // NIEUW: Veilige check voor ingelogd zijn via het Token
  const isLoggedIn = () => !!localStorage.getItem('token');

  // NIEUW: Veilige Admin Check via het JWT Token
  const isRealAdmin = () => {
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            ...
            <nav className="hidden lg:flex items-center gap-8">
              <Link to="/" className={`text-sm font-semibold hover:text-primary transition-colors cursor-pointer uppercase tracking-wide ${isActive('/') ? 'text-primary border-b-2 border-primary pb-1' : ''}`}>Home</Link>
              <Link to="/lijst" className={`text-sm font-semibold hover:text-primary transition-colors cursor-pointer uppercase tracking-wide ${isActive('/lijst') ? 'text-primary border-b-2 border-primary pb-1' : ''}`}>De Lijst</Link>
              <Link to="/artiesten" className={`text-sm font-semibold hover:text-primary transition-colors cursor-pointer uppercase tracking-wide ${isActive('/artiesten') || isActive('/artiest') ? 'text-primary border-b-2 border-primary pb-1' : ''}`}>Artiesten</Link>
              <Link to="/nummers" className={`text-sm font-semibold hover:text-primary transition-colors cursor-pointer uppercase tracking-wide ${isActive('/nummers') || isActive('/nummer') ? 'text-primary border-b-2 border-primary pb-1' : ''}`}>Nummers</Link>
              ...
              <Link to="/geschiedenis" className={`text-sm font-semibold hover:text-primary transition-colors cursor-pointer uppercase tracking-wide ${isActive('/geschiedenis') ? 'text-primary border-b-2 border-primary pb-1' : ''}`}>Geschiedenis</Link>
              <Link to="/statistieken" className={`text-sm font-semibold hover:text-primary transition-colors cursor-pointer uppercase tracking-wide ${isActive('/statistieken') ? 'text-primary border-b-2 border-primary pb-1' : ''}`}>Statistieken</Link>
            </nav>
                    ...
                    <div className="bg-popover text-popover-foreground border border-border rounded-lg shadow-xl py-2 w-56">
                      {isLoggedIn() ? (
                        <>
                          ...
                          {/* DE FIX: KIJK NAAR HET TOKEN, NIET NAAR HET LOSSE WOORDJE */}
                          {isRealAdmin() && (
                            <>
                              <div className="border-t border-border my-1"></div>
          ...
          {mobileMenuOpen && (
            <nav className="lg:hidden py-4 border-t border-border text-white">
              <Link to="/" className={`block py-2 hover:text-primary transition-colors ${isActive('/') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Home</Link>
              <Link to="/lijst" className={`block py-2 hover:text-primary transition-colors ${isActive('/lijst') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>TOP 2000 Lijst</Link>
              <Link to="/artiesten" className={`block py-2 hover:text-primary transition-colors ${isActive('/artiesten') || isActive('/artiest') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Artiesten</Link>
              <Link to="/nummers" className={`block py-2 hover:text-primary transition-colors ${isActive('/nummers') || isActive('/nummer') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Nummers</Link>
              ...
              <Link to="/geschiedenis" className={`block py-2 hover:text-primary transition-colors ${isActive('/geschiedenis') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Geschiedenis</Link>
              <Link to="/statistieken" className={`block py-2 hover:text-primary transition-colors ${isActive('/statistieken') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Statistieken</Link>
              <Link to="/faq" className={`block py-2 hover:text-primary transition-colors ${isActive('/faq') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>FAQ</Link>
              <Link to="/contact" className={`block py-2 hover:text-primary transition-colors ${isActive('/contact') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Contact</Link>
              
              {isLoggedIn() && (
                <Link to="/playlists" className={`block py-2 hover:text-primary transition-colors mt-2 pt-4 border-t border-border ${isActive('/playlists') || isActive('/playlist') ? 'text-primary font-semibold' : ''}`} onClick={() => setMobileMenuOpen(false)}>Mijn Playlists</Link>
              )}

              {isRealAdmin() && (
                <div className="py-2 mt-2 pt-4 border-t border-border">
                  <div className="font-semibold mb-2">Admin</div>
                  <Link to="/admin" className="block py-1 pl-4 text-sm hover:text-primary transition-colors" onClick={() => setMobileMenuOpen(false)}>Admin Panel</Link>
                </div>
              )}

              {isLoggedIn() ? (
                <button className="block w-full text-left py-2 hover:text-primary transition-colors mt-2 pt-4 border-t border-border text-red-500" onClick={() => { localStorage.clear(); window.location.reload(); }}>Uitloggen</button>
              ) : (
                <Link to="/login" className="block py-2 hover:text-primary transition-colors mt-2 pt-4 border-t border-border" onClick={() => setMobileMenuOpen(false)}>Inloggen</Link>
              )}
            </nav>
          )}
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground">De grootste muzieklijst van Nederland</p>
            </div>
            <div>
